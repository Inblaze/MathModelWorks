...
for i=65:100 %步长为1搜索
    ...
    t=0:dt:T;
    u=heat(xm);
    if find(abs(diff(u))>3*0.5) %如果存在斜率大于3的数据
        continue;
    end
    tmp_con=0.5*length(find(u>=150 & u<=190));
    if tmp_con<60 || tmp_con>120 %如果温度上升过程中在150ºC~190ºC的时间不在60~120s内
        continue;
    end
    tmp_con=0.5*length(find(u>217)); 
    if tmp_con<40 || tmp_con>90 %如果存在温度大于217的时间不在40~90s内
        continue;
    end
    tmp_con=max(u);
    if tmp_con<240 || tmp_con>250 %如果存在峰值温度不在240~250
        continue;
    end
    tmp_ans2=[tmp_ans2,i];
end
ans2=max(tmp_ans2);
tmp_ans2=[];
for i=(ans2-1):0.01:(ans2+1)
    v=i/60;
    T=L/v; 
    t=0:dt:T;
    u=heat(xm);
    if ~isempty(find(abs(diff(u))>3*0.5,1)) %如果存在斜率大于3的数据
        continue;
    end
    tmp_con=0.5*length(find(u>=150 & u<=190));
    if tmp_con<60 || tmp_con>120 %如果温度上升过程中在150ºC~190ºC的时间不在60~120s内
...
v=ans2/60;L=50+30.5*11+5*10; T=L/v; dt=0.5;
t=0:dt:T;
u=heat(xm);
figure
plot(t*v,u);
